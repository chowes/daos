		       ...
		       int *result, bool update, bool notify)
{
	...
	if (cpr->cpr_destroyed) {
		notify = true;
		/* Always update in-DRAM bookmark even if it is destroyed. */
		cbk = &cpr->cpr_bk;
		...
		if (rc != 0)
			D_WARN("Failed to delete pool (" DF_UUID ") bookmark after repair: %d\n",
			       DP_UUID(cpr->cpr_uuid), rc);
	} else if (cpr->cpr_skip) {
		cbk = &cpr->cpr_bk;
		if (*result != 0)
...
check_dead:
		...
		if (phase >= CHK__CHECK_SCAN_PHASE__CSP_POOL_MBS && !ins->ci_orphan_done &&
...
static int
chk_leader_query_cb(void *args, uint32_t ins_status, int result, void *data, uint32_t nr)
{
		 ...
		 chk_query_pool_cb_t pool_cb, void *buf)
{
	...
	d_list_for_each_entry(cpr, &ins->ci_pool_list, cpr_link) {
		...
		if (!skip) {
			...
			rc = chk_pool_add_shard(cqa->cqa_hdl, &cqa->cqa_list, cpr->cpr_uuid,
						-1 /* unknown rank */, NULL, ins, &cqa->cqa_count,
						shard, chk_leader_free_shard, NULL);
			if (rc != 0)
				goto out;
		}
	...
	status = chk_ins_merge_status(cbk->cb_ins_status, cqa->cqa_ins_status);
	rc = head_cb(status, cbk->cb_phase, &cbk->cb_statistics, &cbk->cb_time,
		     cqa->cqa_count, buf);
	if (rc != 0)
		goto out;
...
chk_leader_act(uint64_t seq, uint32_t act, bool for_all)
{
	...
	d_list_for_each_entry_safe(pool, tmp, &ins->ci_pool_list, cpr_link) {
		if (rc == 0) {
			ABT_rwlock_wrlock(ins->ci_abt_lock);
			d_list_for_each_entry(pending, &pool->cpr_pending_list, cpr_pool_link) {
				if (pending->cpr_class != act)
					continue;

				rc = chk_leader_act_internal(ins, pending->cpr_seq, act, false,
							     true, NULL);
				if (rc != 0)
					break;
			}

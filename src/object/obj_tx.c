		    ...
		    struct daos_cpd_sub_req **slot)
{
	...
	/* For the case with fixed epoch, it will NOT call dc_tx_get_next_slot(). */
...
dc_tx_close_internal(struct dc_tx *tx)
{
	...
	D_DEBUG(DB_IO, "close TX %p with epoch "DF_X64"\n", tx, tx->tx_epoch.oe_value);
		 ...
		 daos_handle_t *th)
{
	...
	if (rc == 0) {
		*th = dc_tx_ptr2hdl(tx);
		D_DEBUG(DB_IO, "open local TX %p with epoch "DF_X64"\n", tx, epoch);
	}

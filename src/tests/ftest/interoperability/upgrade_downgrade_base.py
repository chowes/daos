...
class UpgradeDowngradeBase(IorTestBase):
    ...
    def full_system_stop(self, servers=True, agents=True):
    ...
    def pool_get_prop(self, pool):
        """Call dmg pool get-prop on a server node.

        Workaround until DAOS-13919 is resolved.

        Args:
            pool (TestPool): pool object
        """
        cmd = f"dmg pool get-prop {pool.identifier}"
        if not run_remote(self.log, self.first_server, cmd).passed:
            self.fail("Failed to get pool properties")
    ...
    def install_daos(self, version, servers, clients):
        ...
        self.full_system_stop(servers=bool(servers), agents=bool(clients))
    ...
    def wait_for_pool_upgrade(self, pool, status, timeout=300):
        ...
        start = time.time()
        while True:
            actual_status = self.get_dmg_command().pool_get_prop(
                pool.identifier, "upgrade_status")['response'][0]['value']
    ...
    def upgrade_and_downgrade(self, fault_on_pool_upgrade=False):
        ...
        self.show_daos_version(all_hosts, hosts_client)
        # TODO verify starting version is as expected
        # TODO verify old_version and new_version RPMs are available upfront
